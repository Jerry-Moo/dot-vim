function! etc#util#error(msg) abort
	for l:mes in etc#util#str2list(a:msg)
		echohl WarningMsg | echomsg '[nvim-etc] ' . l:mes | echohl None
	endfor
endfunction
...
function! etc#util#load_config(filename) abort
    ...
    elseif a:filename =~# '\.ya\?ml$'
        " Parse YAML with common command-line utilites
        return etc#util#load_yaml(a:filename)
    endif
    call etc#util#error('Unknow config file format ' . a:filename)
    return ''
endfunction
