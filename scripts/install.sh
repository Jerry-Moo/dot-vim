...
action "Create .dotvim.d "

function ensureDotvimd(){
  if [ ! -d "$dotvim_personal" ]
  then
    mkdir -p $HOME/.dotvim.d
    ok "create .dotvim.d folder success"
  else
    warn "the .dotvim.d folder exist skipped"
  fi
  ...
  then
    touch init.vim
    ok "create .dotvim.d/init.vim success"
  else
    warn "the .dotvim.d/init.vim exist skipped"
  fi

  if [ ! -f plugins.yaml ]
  then
    touch plugins.yaml
    ok "create .dotvim.d/plugins.yaml success"
    cd -
    cd $HOME/.config/nvim/
    action "Building generator binary"
    go build -o bin/generator cmd/main.go
    action "choose your languages"
    ./bin/generator
    cd -
  else
    warn "the .dotvim.d/plugns.yaml exist skipped"
  fi
}
